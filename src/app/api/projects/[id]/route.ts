// app/api/projects/[id]/route.ts (UPDATED with PUT handler)
...
// Helper function to extract the ID from the URL path
...
export async function PUT(req: Request) {
    try {
        ...
        const updatedProject = await Project.findByIdAndUpdate(
            id,
            { $set: updateData }, // Update all fields passed in the body
            { new: true, runValidators: true } // Return the new document, run schema validation
        );
        ...
        return NextResponse.json({ error: "Server error or validation failed", details: String(err?.message || err) }, { status: 500 });
    }
}


// 📌 GET /api/projects/[id] (Unchanged, fetches data for the edit form)
export async function GET(req: Request, { params }: { params: { id: string } }) {
    // ... (Your existing GET logic using getIdFromUrl(req.url) here)
    try {
        await requireAdmin(req);
